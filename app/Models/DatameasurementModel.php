...
class DatameasurementModel extends Model
{
	...
	protected $allowedFields = ['user_id', 'date_id', 'position_id', 'station_start_id', 'station_end_id', 'lat', 'lng', 'amplitude_x', 'amplitude_y', 'amplitude_z', 'y_per_z', 'recorded_at'];

	public function getFilter($user_id, $start, $end, $checked)
	{
		$builder = $this->table('data_measurement');
		$builder->select('data_measurement.id as dataid, lat, lng, amplitude_y, amplitude_z, y_per_z, date, name')
